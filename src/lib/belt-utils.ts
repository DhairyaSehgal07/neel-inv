...
import { Belt } from "./data";
import { BeltDocument } from "@/model/Belt";

/**
 * Convert MongoDB Belt document to frontend Belt format
 * Converts _id to id and handles date formatting
 */
export function convertBeltDocumentToBelt(doc: BeltDocument | null): Belt | null {
  if (!doc) return null;
 ...
 */
export function convertBeltDocumentsToBelts(docs: BeltDocument[]): Belt[] {
